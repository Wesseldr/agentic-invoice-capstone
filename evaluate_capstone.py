...
def evaluate_invoice(generated_path, ground_truth_path):
    ...
    if gen_no_act == truth_no_act:
        ...
        report.append(f"❌ NoActivity mismatch.")
...
def run_evaluation():
    ...
    print("📊 STARTING CAPSTONE EVALUATION\n")
    ...
    for truth_file in truth_files:
        ...
        if not gen_file.exists():
            print(f"⚠️ Missing generated file for `{truth_file.name}`. Tried looking for: `{generated_name}`")
            continue
            
        acc, report = evaluate_invoice(gen_file, truth_file)
        results.append(acc)
        
        print(f"📄 {truth_file.name}: {acc:.1f}%")
        for line in report:
            print(f"   {line}")
